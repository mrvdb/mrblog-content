...
#+BEGIN_SRC sh :exports code :tangle ~/bin/edit :shebang #!/bin/bash
  ...
  EMACSCLIENT="`which emacsclient` --quiet --no-wait"
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq el-get-sources
        '(
          ...
          ;;(:name mrb/ubiquitous       :type github :pkgname "DarwinAwardWinner/ido-ubiquitous")
          (:name guide-key              :type github :pkgname "kbkbkbkb1/guide-key")
          (:name org-screenshot         :type github :pkgname "dfeich/org-screenshot")
          (:name erc-hl-nicks           :type github :pkgname "leathekd/erc-hl-nicks")
  ...
  (setq mrb/packages
        (append
         ...
         '(smex ace-jump-mode apache-mode gnuplot-mode sudo-save
                ...
                identica-mode php-mode company-mode xml-rpc csv-mode notmuch use-package git-timemachine)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq user-domain "hsdev.com") ;; This one I made up.
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun mrb/unicode-font-corrections()
    ...
    (mrb/fix-fontset-font "Symbola" #x1F4A9) ;; 💩  pile of poo is required
    (mrb/fix-fontset-font "Symbola" #x1f44d) ;; 👍  thumbs up
   )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (if
    ...
    (bind-key "C-c e" 'org-export org-mode-map)
    )
...
#+BEGIN_SRC emacs-lisp
  ;(autoload 'haskell-mode "haskell-mode" "Haskell support" t)
...
;; C-c C-space has the control key in it, so this needs redefinition
(global-set-key [(control c) (control key-4660)] 'erc-track-switch-buffer)
...
#+BEGIN_SRC emacs-lisp
  ...
  (if (not (fboundp 'toggle-frame-fullscreen))
      (defun mrb/toggle-frame-fullscreen()
        (interactive)
    ...
    (setq
     ...
     (quote
      (autojoin button completion fill
       ...
       match move-to-prompt netsplit
       networks noncommands notify notifications
       readonly replace ring services smiley stamp track unmorse
         smiley image truncate))
...
#+BEGIN_SRC emacs-lisp
    ...
    (setq erc-nick '("_MrB_" "_MrB__"))

    (setq erc-prompt-for-nickserv-password nil)
    (setq erc-nickserv-passwords
          `((freenode     (
                           ("_MrB_" . "M9UDV2Lk+BsG68uabZ1Kplh3")
                           ))))
    (erc-services-mode 1)
    (erc-truncate-mode 1)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (setq erc-autojoin-channels-alist
        '((".*\\.freenode.net" "#openobject" "#social" "#bitcoin-nl" "#twister")))
#+END_SRC
...
Especially in long running sessions on IRC, nick notifications are
essential. I am on emacs 24 and this has DBUS notifications built-in,
so let's start to use that. I'd rather have a graceful fallback (like
notify.el) but we can use that later on. For now, the builtin
notifications is preferred, so I don't have to install anything extra

#+BEGIN_SRC emacs-lisp
  (require 'notifications)

  (defun mrb/erc-notify (match-type nick message)
    "Notify when a message is received on erc"
  ...
  (add-hook 'erc-text-matched-hook 'mrb/erc-notify)
#+END_SRC

This is the simplest I can come up with for now. It notifies a bit too
much, like server messages etc.
...
#+BEGIN_SRC emacs-lisp
  (setq
   ; User agent style is message mode (gnus, but independent of it)
    ...
    smtpmail-smtp-service 24 ;; I use anubis for pre-processing
...
*** GNUS
    ...
    It's probably going to be an iterative effort to get things
    working the way I want them to, so I'm going to configure things
    from a working situation to a new working situation where the
    latte does not necessarily represent a wanted situation, but takes
    me close to it.
...
#+BEGIN_SRC emacs-lisp
  ; Directly tie into the GIT repository on my machine because I have
  ; made some modifications which are workarounds
  (add-to-list 'load-path "/home/mrb/dat/src/emacs/packages/jabber.el/")
  (require 'jabber)
  ...
  (setq jabber-account-list (quote (("marcel@hsdev.com/Emacs" (:connection-type . starttls)))))
...
clumsy, although very flexible.

#+BEGIN_SRC emacs-lisp
  ;; Most of what is below is directly inspired by the config of the
  ;; author of elfeed.

  ;; Prevent unneeded repetition in specifying feeds
  (makunbound 'feed-patterns)
  (defvar feed-patterns
    '((marktplaats "http://kopen.marktplaats.nl/opensearch.php?q=%s")
      (youtube     "https://gdata/youtube.com/feeds/base/users/%s/uploads")
      (reddit      "https://reddit.com/r/%s/.rss")))

  ;; Bulk of the feed specification
  (makunbound 'elfeed-feeds-alist)
  (defvar elfeed-feeds-alist
        '(("http://gdata.youtube.com/feeds/base/users/mvanderboom/newsubscriptionvideos")
          ("bridgeport" marktplaats machining)
          ("abene" marktplaats machining)
          ("plasmasnijder" marktplaats machining)
          ("cazeneuve" marktplaats machining)
          ("deckel" marktplaats machining)
          ("lista" marktplaats machining)
          ("x200s" marktplaats computing)
          ...
          ("http://mrblog.nl/feed" me blog )
          ("http://www.flixfilms.nl/feed" netflix info)
          ("http://feeds.feedburner.com/TheDailyBitcoinShow" bitcoin )
          ("http://feeds.feedburner.com/GeekAndPoke")
          ("http://blag.xkcd.com/feed/" humor )
          ("http://www.xkcd.com/rss.xml" humor comic)
          ("http://extechops.net/full-episode-feeds/daily-show.rss" video humor)
          ("http://www.nu.nl/feeds/rss/de-jager.rss" humor comic)
          ("http://feeds.feedburner.com/oatmealfeed" humor comic)
          ("http://what-if.xkcd.com/feed.atom" humor comic science)
          ("https://kimmo.suominen.com/stuff/dilbert-daily.xml" humor comic)
          ("http://www.commitstrip.com/en/feed/" comic humor programming)
          ("http://feeds.feedburner.com/TheChicane" oldschool )
          ("http://gd427cobrabuild.blogspot.com/feeds/posts/default" cobra)
          ("http://buildingmycobra.blogspot.com/feeds/posts/default"  cobra)
          ("http://www.cobraclub.com/forum/external.php?type=RSS2" cobra)
          ("http://www.sumobuild.co.uk/xml/sumobuild.xml" cobra)
          ("http://www.gdcars.com/feed" cobra)
          ("http://forum.cobraclub.nl/index.php?type=rss;action=.xml" cobra)
          ("http://isispower.blogspot.com/feeds/posts/default" cobra)
          ("http://andysgd427.blogspot.com/feeds/posts/default" cobra)
          ("http://gd-cobra.blogspot.com/feeds/posts/default" cobra)
          ("http://robscobra.blogspot.com/feeds/posts/default" cobra)
          ("http://ryan427.blogspot.com/feeds/posts/default" cobra)
          ("http://str-427-cobra.blogspot.com/feeds/posts/default" cobra)
          ("http://cobnut.blogspot.com/feeds/posts/default" cobra)
          ("http://cobra.mrblog.nl/feed" cobra me)
...
;;          ("http://support.lenovo.com/en_US/rss/default.page?PROD=P014-S006-SS0066--")
          ...
          ("http://www.garron.me/en/atom.xml" people emacs jekyll)
          ("http://technomancy.us/feed/atom.xml" people emacs)
          ("http://nullprogram.com/feed/" people emacs)
          ...
          ("thinkpad" reddit computing)
          ("http://www.emacswiki.org/emacs/index.rss?action=rss" emacs software)
          ("http://feeds.feedburner.com/hacknmod/qjUG")
          ("https://blog.sleeplessbeastie.eu/atom.xml")
          ("http://esr.ibiblio.org/?feed=atom" people )
          ("http://esr.ibiblio.org/?feed=atom&tag=gpsd")
          ("http://feeds2.feedburner.com/TheArtOfManliness")
          ;; Add new feeds above this
          ))

  ;; Expand what is in the feeds-alist, taking the patterns into account
  ...
  ;; Allow the hook to run our tags
